...
static void *get_next_frame_buffer(esp_lcd_panel_handle_t panel_handle)
{
    ...
    if (next_fb == NULL)
    {
        lvgl_get_lcd_frame_buffer(panel_handle, 2, &fb[0], &fb[1]);
        next_fb = fb[1];
    }
    else
    {
...
IRAM_ATTR static void rotate_copy_pixel(const uint16_t *from, uint16_t *to, uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end, uint16_t w, uint16_t h, uint16_t rotation)
{
    ...
    ppa_srm_oper_config_t oper_config = {
        .in = {
            ...
            .pic_w = w,
            .pic_h = h,
            .block_w = x_end - x_start + 1,
            .block_h = y_end - y_start + 1,
            .block_offset_x = x_start,
            .block_offset_y = y_start,
            .srm_cm = (LV_COLOR_DEPTH == 24) ? PPA_SRM_COLOR_MODE_RGB888 : PPA_SRM_COLOR_MODE_RGB565,
        },
        .out = { 
            .buffer = to,
            .buffer_size = ALIGN_UP_BY(sizeof(lv_color_t) * w * h, data_cache_line_size),
            .pic_w = (ppa_rotation == PPA_SRM_ROTATION_ANGLE_90 || ppa_rotation == PPA_SRM_ROTATION_ANGLE_270) ? h : w,
            .pic_h = (ppa_rotation == PPA_SRM_ROTATION_ANGLE_90 || ppa_rotation == PPA_SRM_ROTATION_ANGLE_270) ? w : h,
            .block_offset_x = x_offset,
            .block_offset_y = y_offset,
            .srm_cm = (LV_COLOR_DEPTH == 24) ? PPA_SRM_COLOR_MODE_RGB888 : PPA_SRM_COLOR_MODE_RGB565,
        },
...
{
    return get_next_frame_buffer((esp_lcd_panel_handle_t) panel_handle);
}
...
static lv_display_t *display_init(esp_lcd_panel_handle_t panel_handle)
{
    ...
    esp_cache_get_alignment(MALLOC_CAP_DMA | MALLOC_CAP_SPIRAM, &data_cache_line_size);
#endif
    ...
    lvgl_get_lcd_frame_buffer(panel_handle, 3, &lvgl_port_rgb_last_buf, &buf1, &buf2);
    ...
    buf1 = fbs[2];
#else
    lvgl_get_lcd_frame_buffer(panel_handle, 2, &buf1, &buf2);
#endif
#else
...
esp_err_t lvgl_port_init(esp_lcd_panel_handle_t lcd_handle, esp_lcd_touch_handle_t tp_handle, lvgl_port_interface_t interface)
{
    ...
    if (ret != pdPASS)
    {
        ESP_LOGW(TAG, "Failed to create LVGL task");
        return ESP_FAIL;
    }
